 ...
 * @icon fa fa-circle-o
 */
class Exchangeshuanhui extends Backend
{
    
    /**
     * Exchangeshuanhui模型对象
     * @var \app\admin\model\Exchangeshuanhui
     */
    ...
    {
        parent::_initialize();
        $this->model = new \app\admin\model\Exchangeshuanhui;
    ...
    public function index()
    {
        ...
        if ($this->request->isAjax())
        {
            ...
            foreach ($list as $row) {
                $row->visible(['id','Redeemtheaccount','status','Redeemintotheaccount','dingdanhao','CashoutAmount','Redemptioamount','exchangerate','Handlingee','Redeemtheaccountqmoney','Redeemtheaccounthmoney','Redeemintotheaccountqmoney','Redeemintotheaccounthmoney','beizhu','createtime','updatetime','operator']);
		    ...
		    public function add()
    {
        if ($this->request->isPost()) {
			...
			$params["Redeemtheaccounthmoney"] = $params["Redeemtheaccountqmoney"] - $params["CashoutAmount"] - $params["Handlingee"];
			$params["Redeemintotheaccountqmoney"] = action('Incomerecord/zhuanchuqianjieyue',$params["Redeemintotheaccountid"]);
			$params["Redeemintotheaccounthmoney"] = $params["Redeemintotheaccountqmoney"] +  $params["Redemptioamount"];
         ...
         if(round(action('Incomerecord/zhangbianhou',$params["Redeemintotheaccountid"]),2) != round($params["Redeemintotheaccountqmoney"],2)){
			...
			//var_dump($params);exit;
				...
				$datas['Amount'] =  -$params['CashoutAmount'];
				...
				$datas['Amount'] =  $params['Redemptioamount'];
				$datas['operator'] = $params['operator'];
				$datas['time'] = $params['createtime'];
				$datas['status']	= 4; 
				$this->zhangbianruku($datas);	
	    ...
	    public function delete($ids = "")
    {
			...
			$adminIds = explode(',',$params["ids"]);
			$where = [];
			$where['id'] = ['in',$adminIds];
			$int =  $this->model->where($where)->delete();
		    if($int){
